...
enum NRF_ERRORS {NRF_OK = E_OK,
	NRF_PTXONLY, // function only available in PTX mode
	NRF_TXOVERLAP, // attemp to transmit while already transmitting
	NRF_PRXONLY, // function only available in PRX mode
	NRF_N_ERR_CODES
};

extern char *nrf_messages[];
...
int nrf_Transmit (void *data, size_t length, nrf_PTXptr eot);
// Initializes a transmission to Pipe 0.
// Only 32 bytes can be transmitted at once. If a longer transmission is requested
// only the first 32 bytes are transmitted.
// Input:
//	data: pointer to the data to be transmitted
//	length: size (in bytes) of the data block
//	eot: Callback to be called after the transmission ends, with parameters:
//		success: true if an ACK packet was received from the PRX, and
//			false if the maximum number of retransmissions have occured.
//		ackPayload: If the PRX answers with payload, it is stored here
//		length: the payload's length
// 	ackPayload must be read before any call to any other nrf function is done.
// Returns:
//	Upon success: The number of bytes transmitted (nonnegative)
//	Upon failure: minus the error code
// nrf_Transmit throws an error if the module is not initialized as PTX, or if
// a transmission is taking place. When eot is called, a new transmission can be initiated.
...
int nrf_Receive (nrf_PRXptr eot);
...
//	Upon failure, returns minus the error code (defined in 'enum NRF_ERRORS')
