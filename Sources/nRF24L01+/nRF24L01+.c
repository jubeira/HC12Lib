...
char *nrf_messages[] = {
/*NRF_OK*/ "Ok",
/*NRF_PTXONLY*/	"nrf: function only available in PTX mode",
/*NRF_TXOVERLAP*/ "nrf: attempted to initiate a transmission while another one is taking place",
/*NRF_PRXONLY*/ "nrf: Receive can only be called in PRX mode"
};
...
void nrf_InitSequence (void)
{
	switch (nrf_initStep)
	{
		...
		case 4:
			...
			nrf_data.spiInputData[1] = ARD(1) | ARC(0);
...
{
	if (nrf_data.type != PTX)
		PUTS_WARNING("nrf: IsBusy is only available in PTX mode.\n");
...
int nrf_Transmit (void *data, size_t length, nrf_PTXptr eot)
{
	...
	if (nrf_data.type != PTX) {
		return -NRF_PTXONLY;
	} else if (nrf_data.ptxData.transmitting == _TRUE) {
		return -NRF_TXOVERLAP;
	} else {
		if (data == NULL)
			PUTS_WARNING("nrf: recevied NULL data pointer.\n");

		nrf_data.ptxData.eot = eot;

		real_length = (length > 32)? 32 : length;

		if (real_length == 0) {
			PUTS_WARNING("nrf: attempt to send a 0 length message. Did nothing\n");
		} else {
			nrf_data.ptxData.transmitting = _TRUE;
			nrf_CommenceTransmission (data, real_length);
		}
		return real_length;
	}
}
...
void nrf_CommenceTransmission (u8 *data, u8 length)
{
	...
	memcpy(nrf_data.spiInputData + 1, data, length*sizeof(*data));
...
int nrf_Receive (nrf_PRXptr eot)
{
	...
	if (eot == NULL)
		PUTS_WARNING("nrf: received a null end of transmission callback.\n");

	if (nrf_data.prxData.eot != NULL)
		PUTS_WARNING("nrf: attempted to register a second end of transmission callback.\n");
